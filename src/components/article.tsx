    ...
    toggleTranslation = async () => {
        ...
        if (this.state.showTranslation) {
            ...
            try {
                ...
                console.log('[Article Translate] Translating title:', this.props.item.title)
                const { translateText } = await import('../scripts/translate')
                const translatedTitle = await translateText(this.props.item.title)
                console.log('[Article Translate] Translated title:', translatedTitle)

                // Translate content
                console.log('[Article Translate] Translating content...')
                const translation = await translateHtml(content)
                console.log('[Article Translate] Content translation complete')

                // Translate summary if exists
                let translatedSummary = null
                if (this.state.summary) {
                    console.log('[Article Translate] Translating summary:', this.state.summary.substring(0, 100))
                    translatedSummary = await translateText(this.state.summary)
                    console.log('[Article Translate] Translated summary:', translatedSummary)
                }
    ...
    articleView = () => {
        ...
        const h = encodeURIComponent(
            renderToString(
                <>
                    <p className="title">
                        {this.state.showTranslation && this.state.translatedTitle
