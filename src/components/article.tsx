...
class Article extends React.Component<ArticleProps, ArticleState> {
    ...
    toggleTranslation = async () => {
        ...
        if (this.state.showTranslation) {
            ...
            // Check if translation mode has changed
            const currentMode = window.settings.getIntegrationSettings()?.translationMode || "full"
            const storedMode = this.state.translationMode || "full"
            
            if (currentMode !== storedMode) {
                // Translation mode changed, need to re-translate
                console.log('Translation mode changed from', storedMode, 'to', currentMode)
                this.setState({ translating: true, translationProgress: 0, translationTotal: 0 })
                try {
                    ...
                    const { translateHtml } = await import('../scripts/translate')
                    const translation = await translateHtml(content, undefined, (current, total) => {
                        this.setState({ translationProgress: current, translationTotal: total })
                    })
                    this.setState({
                    ...
                    this.setState({ translating: false, translationProgress: 0, translationTotal: 0 })
                }
            } else {
                // Mode unchanged, just show existing translation
            ...
            try {
                ...
                // First, translate the title (if not already translated)
                const store = (window as any).__STORE__
                const existingTranslation = store?.getState()?.app?.titleTranslations?.[this.props.item._id]

                if (!existingTranslation) {
                    // Translate title using translateText function
                    const { translateText } = await import('../scripts/translate')
                    const translatedTitle = await translateText(this.props.item.title)

                    // Store in Redux
                    if (store && translatedTitle !== this.props.item.title) {
                        const { translateComplete } = await import('../scripts/models/app')
                        store.dispatch(translateComplete([translatedTitle], [this.props.item._id]))
                    }
                }

                // Then translate the content with progress
                const { translateHtml } = await import('../scripts/translate')
                const translation = await translateHtml(content, undefined, (current, total) => {
                    this.setState({ translationProgress: current, translationTotal: total })
                })
    ...
    articleView = () => {
        ...
        // Translate title if translation is enabled
        let title = this.props.item.title
        if (this.state.showTranslation && this.state.translation) {
            // Get translated title from Redux store
            const translatedTitle = (window as any).__STORE__?.getState()?.app?.titleTranslations?.[this.props.item._id]
            if (translatedTitle) {
                    ...
                    <CommandBarButton
                        title={
                            this.state.translating
                                ? `${intl.get("article.translating") || "Translating..."} ${this.state.translationTotal > 0 ? `(${this.state.translationProgress}/${this.state.translationTotal})` : ""
                                }`
                                : intl.get("article.translate") || "Translate"
                        }
                        className={`${this.state.showTranslation ? "active" : ""} ${this.state.translating ? "translating" : ""}`}
                        iconProps={{
                            iconName: this.state.translating ? "ProgressRingDots" : "Dictionary",
                            style: this.state.translating ? { animation: "spin 1s linear infinite" } : {}
                        }}
                        disabled={
                        ...
                        onRenderText={() => (
                            <>
                                <span>{this.state.translating
                                    ? `${intl.get("article.translating") || "Translating..."} ${this.state.translationTotal > 0 ? `(${this.state.translationProgress}/${this.state.translationTotal})` : ""
                                    }`
                                    : intl.get("article.translate") || "Translate"
                                }</span>
                                {this.state.translating && this.state.translationTotal > 0 && (
                                    <div className="translation-progress-container">
                                        <div
                                            className="translation-progress-bar"
                                            style={{ width: `${(this.state.translationProgress / this.state.translationTotal) * 100}%` }}
                                        />
                                    </div>
                                )}
                            </>
