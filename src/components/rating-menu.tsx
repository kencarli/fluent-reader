...
import { FilterType, ALL } from "../scripts/models/feed"
...
export default function RatingMenu({ target, onDismiss, position, event }: RatingMenuProps) {
    ...
    const handleFilterChange = async (filterType: number) => {
        ...
        // Check if user is in a valid view for rating filter
        if (feedId === 'SOURCE') {
            alert('请先选择一个具体的订阅源或订阅组')
            onDismiss()
            return
        }
    ...
    const handleRateFeed = async (date: Date | null = null) => {
        ...
        try {
            ...
            let sourceIds = getSourceIdsFromCurrentFeed(storeState)
