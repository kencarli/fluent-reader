...
import { loadAutomationRules } from "../../scripts/automation-service"
...
class RulesTab extends React.Component<RulesTabProps, RulesTabState> {
    ...
    ruleColumns = (): IColumn[] => [
        ...
        },
        {
            ...
            onRender: () => {
                // 统计当前订阅源的自动化规则数量
                const sourceId = this.state.sid ? parseInt(this.state.sid) : null;
                if (!sourceId) return <span style={{ color: 'var(--neutralTertiary)' }}>-</span>;
                
                const rules = loadAutomationRules();
                const ruleCount = rules.filter(r => 
                    r.condition.sourceIds?.includes(sourceId)
                ).length;
                ...
                return ruleCount > 0 ? (
                    ...
                    <span style={{ color: 'var(--neutralTertiary)' }}>
                        无关联
                    </span>
                );
            }
