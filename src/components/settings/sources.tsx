...
class SourcesTab extends React.Component<SourcesTabProps, SourcesTabState> {
    ...
    componentDidUpdate = (prevProps: SourcesTabProps) => {
        // 当从其他页面切换回来时，如果订阅源数据变化，强制更新
        if (prevProps.sources !== this.props.sources) {
            this.forceUpdate()
        }
        // 如果订阅源数量变化，也强制更新
        if (Object.keys(prevProps.sources).length !== Object.keys(this.props.sources).length) {
            this.forceUpdate()
        }
    }
            ...
            onRender: (s: RSSSource) => {
                ...
                return (
                    <Stack horizontal verticalAlign="center" tokens={{ childrenGap: 2 }}>
                        ...
                        />
                        {timeAgo && (
                            <span style={{ fontSize: 9, color: 'var(--neutralTertiary)' }}>{timeAgo}</span>
                        )}
                    </Stack>
                )
            }
