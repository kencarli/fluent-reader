...
export class AppState {
    ...
    digestOn = false
    digestHours?: number  // Selected time range for digest (24, 72, 168, etc.)
    translateOn = false
    translating = false
    translateProgress = { completed: 0, total: 0 }
    titleTranslations: { [itemId: number]: string } = {}
...
export interface ToggleDigestAction {
    ...
    hours?: number  // Optional: time range in hours (24, 72, 168, etc.)
}
...
export const toggleDigest = (hours?: number) => ({ type: TOGGLE_DIGEST, hours })
...
export const translateComplete = (translatedTitles: string[], itemIds: number[]) => ({
    type: TRANSLATE_COMPLETE,
    payload: { translatedTitles, itemIds }
})
...
export function setupScheduledDigest(): AppThunk {
    return (dispatch, getState) => {
        ...
        digestInterval = setInterval(async () => {
            ...
            const hasLLMProvider = settings.openaiApiKey || settings.nvidiaApiKey || settings.deepseekApiKey || (settings.ollamaApiUrl && settings.ollamaModel)
    ...
    return dispatch => {
        ...
        dispatch(initIntl())
            ...
            .then(() => {
                ...
                const { startScheduledRules } = require('../automation-service')
                startScheduledRules()
            })
            .catch(err => {
                console.error("Failed to initialize app:", err)
                // Still set locale to avoid blank screen
                dispatch(initIntlDone(getCurrentLocale()))
            })
    }
}
    ...
    switch (action.type) {
        case TOGGLE_DIGEST:
            return {
                ...
                digestHours: action.hours  // Store the selected time range
            }
        ...
        case TRANSLATE_COMPLETE:
            // Store translations in Redux state
            const translations = { ...state.titleTranslations }
            action.payload.itemIds.forEach((itemId, index) => {
                const translatedTitle = action.payload.translatedTitles[index]
                if (translatedTitle) {
                    translations[itemId] = translatedTitle
                }
            })
            return {
                ...state,
        ...
        case OPEN_MARK_ALL_MENU:
            return {
                ...
                contextMenu: {
                    ...
                    event: action.event || "#mark-all-toggle",
                },
            }
        case TOGGLE_MENU:
            return {
