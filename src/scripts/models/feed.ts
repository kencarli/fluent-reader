...
export class FeedFilter {
    ...
    static toPredicates(filter: FeedFilter) {
        // Defensive check: ensure db.items is initialized
        if (!db.items) {
            console.error("Database not initialized: db.items is undefined")
            return []
        }
...
export class RSSFeed {
    ...
    static async loadFeed(feed: RSSFeed, skip = 0): Promise<RSSItem[]> {
        ...
        if (!db.itemsDB || !db.items) {
            console.error("Database not initialized: cannot load feed")
            return []
        }
        ...
        case SELECT_PAGE:
            switch (action.pageType) {
                case PageType.Sources:
                    // Use menuKey as feedId for source selection (e.g., "s-1" or "g-1")
                    const sourceFeedId = action.menuKey || SOURCE
                    return {
                        ...state,
                        [sourceFeedId]: new RSSFeed(
                            sourceFeedId,
                            action.sids,
                            action.filter
                        ),
