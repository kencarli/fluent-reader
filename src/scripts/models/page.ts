...
const applyFilterDone = (filter: FeedFilter): PageActionTypes => ({
    ...
    filter: {
        ...
        ratedItemIds: filter.ratedItemIds ? [...filter.ratedItemIds] : undefined,
        highRatedItemIds: filter.highRatedItemIds ? [...filter.highRatedItemIds] : undefined,
        unratedOnly: filter.unratedOnly,
    }
})

export function applyFilter(filter: FeedFilter): AppThunk {
    return (dispatch, getState) => {
        const oldFilterType = getState().page.filter.type
        // Remove rating bits before saving to settings (rating filters are temporary)
        const RATING_MASK = 128 | 256  // RatedOnly | HighRated
        const filterTypeToSave = filter.type & ~RATING_MASK
        if (filterTypeToSave !== oldFilterType)
            window.settings.setFilterType(filterTypeToSave)
