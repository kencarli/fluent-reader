...
export function initSources(): AppThunk<Promise<void>> {
    return async dispatch => {
        ...
        try {
            ...
            if (!db.sourcesDB || !db.sources || !db.itemsDB || !db.items) {
                throw new Error("Database initialization failed: tables are undefined")
            }
            ...
            // Don't dispatch success with empty state - let the error propagate
            // This prevents the app from continuing in a broken state
        }
    }
}
