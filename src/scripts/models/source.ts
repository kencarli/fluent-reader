...
export class RSSSource {
    ...
    private static async checkItem(
        ...
        // 去重检查：优先使用 link，其次 title+date
        // 1. 首先检查 link（最准确）
        if (i.link) {
            const itemsByLink = (await db.itemsDB
                .select()
                .from(db.items)
                .where(
                    lf.op.and(
            ...
            // 如果 link 已存在，直接返回 null（重复）
            if (itemsByLink.length > 0) {
                return null
            }
        }
        
        // 2. 如果没有 link 或 link 不重复，再检查 title+date
        const itemsByTitle = (await db.itemsDB
            .select()
            .from(db.items)
            .where(
                lf.op.and(
