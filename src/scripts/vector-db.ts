    ...
    try {
        ...
        try {
            ...
            if (validationError.code === 516) {
                console.log('[VectorDB] Validation failed, using memory database...')
        ...
        if (error.code === 300 || error.code === 516) {
            console.log('[VectorDB] Database version mismatch detected')
            console.log('[VectorDB] Using memory database...')
            
            // Fallback to memory database
            try {
                const memorySchema = lf.schema.create(VECTOR_DB_NAME, VECTOR_DB_VERSION)
                memorySchema
                    .createTable(VECTOR_TABLE_NAME)
                    .addColumn("itemId", lf.Type.INTEGER)
                    .addPrimaryKey(["itemId"])
                ...
                // @ts-ignore
                vectorDB = await memorySchema.connect()
                vectors = vectorDB.getSchema().table(VECTOR_TABLE_NAME)
                console.log('[VectorDB] Memory database initialized successfully')
                console.log('[VectorDB] NOTE: Your existing vector data in IndexedDB is safe')
                console.log('[VectorDB] NOTE: Vectors will not persist between sessions')
                return
            } catch (memoryError: any) {
                console.error('[VectorDB] Memory database initialization failed:', memoryError)
                throw memoryError
            }
        } else {
            throw error
